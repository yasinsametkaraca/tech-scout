"""``ts-render-doc`` — render a single Jinja2 template to a markdown file."""
...
from tech_scout.cli._common import parse_path, repo_root, run_script
...
def main() -> dict[str, Any]:
    ...
    if not templates_root.is_dir():
        # Fall back to repo-root/src/tech_scout/templates (handy in dev)
        fallback = repo_root() / "src" / "tech_scout" / "templates"
        if fallback.is_dir():
