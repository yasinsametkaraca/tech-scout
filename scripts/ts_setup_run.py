"""tech-scout setup-run — create an output folder and initial run state.
...
Creates ``<output-folder>`` (if needed) and ``<output-folder>/.tech-scout/<run-id>/``
with ``state.json`` containing the validated :class:`ResearchRequest` and a
fresh :class:`RunSnapshot`. Idempotent unless ``--force`` is passed: a second
call returns the existing run-id rather than creating a new one.

Output: JSON envelope with the run_id, output_folder, state_dir, and the
resolved locale codes (canonical 2-letter codes after alias normalization).
"""
...
def main() -> dict[str, Any]:
    ...
    # Resolve locale early so we can fail fast on unknown codes and pin the
    # canonical 2-letter form into state for the rest of the pipeline.
    ...
    # If a run-id already exists for this folder and not forced, reuse it
